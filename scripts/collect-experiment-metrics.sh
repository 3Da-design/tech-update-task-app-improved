...
# S0 Blade のフロントは resources 配下（Blade テンプレート + JS/CSS）。バックエンドはサーバ + テスト。
REPOSITORY="stack-s0"
FRONTEND_PATHSPEC=(resources/views resources/js resources/css)
...
doc = {
    ...
    "git_frontend": {
        ...
        "includes": ["resources/views/", "resources/js/", "resources/css/"],
    },
    "git_backend": {
        "diff_ref": os.environ.get("DIFF_REF", "") or None,
